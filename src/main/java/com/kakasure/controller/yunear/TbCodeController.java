...
public class TbCodeController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增TbCode");
		
		pd = this.getPageData();
		pd.put("TBCODE_ID", this.get32UUID());	//主键
		tbcodeService.save(pd);
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除TbCode");
		
		try{
			pd = this.getPageData();
			tbcodeService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改TbCode");
		
		pd = this.getPageData();
		tbcodeService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page){
		logBefore(logger, "列表TbCode");
		
		try{
			pd = this.getPageData();
			page.setPd(pd);
			List<PageData>	varList = tbcodeService.list(page);	//列出TbCode列表
			getHC(); //调用权限
			mv.setViewName("yunear/tbcode/tbcode_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增TbCode页面");
		
		pd = this.getPageData();
		try {
			mv.setViewName("yunear/tbcode/tbcode_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改TbCode页面");
		
		pd = this.getPageData();
		try {
			pd = tbcodeService.findById(pd);	//根据ID读取
			mv.setViewName("yunear/tbcode/tbcode_edit");
	...
	public Object deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				tbcodeService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出TbCode到excel");
		...
		try{
			...
			List<PageData> varOList = tbcodeService.listAll(pd);
