package com.kakasure.controller.system.zidian;
...
import com.kakasure.service.system.zidian.ZidianService;
...
public class ZidianController extends BaseController {
	...
	public ModelAndView save(PrintWriter out) throws Exception {
		...
		if (null == pd.getString("ZD_ID") || "".equals(pd.getString("ZD_ID"))) {
			if (null != PARENT_ID && "0".equals(PARENT_ID)) {
				...
				pd.put("P_BM",
						pdp.getString("BIANMA") + "_" + pd.getString("BIANMA"));
			}
	...
	public void del(PrintWriter out) {
		try {
			pd = this.getPageData();
			if (Integer.parseInt(zidianService.findCount(pd).get("ZS")
					.toString()) != 0) {
				out.write("false");
			} else {
